    ...
    <div className={`flex ${showAiAssistant ? 'space-x-6' : ''}`}>
      <div className={`${showAiAssistant ? 'w-2/3' : 'w-full'} space-y-6`}>
        ...
        <div className="flex justify-between items-center">
          ...
          <button className="px-3 py-1.5 text-sm font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/40 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/60 flex items-center transition-colors duration-200" onClick={handleAiAssistantToggle}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="md:col-span-1">
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4 h-full flex flex-col">
              <h4 className="text-sm font-medium text-gray-800 dark:text-white mb-3">
                {isRTL ? 'تحميل ملف جديد' : 'Upload New File'}
              </h4>
              <div className="border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-6 flex flex-col items-center justify-center flex-grow">
                ...
                <button className="px-4 py-2 text-sm font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/40 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/60" onClick={handleFileUpload}>
          ...
          <div className="md:col-span-2">
            <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              ...
              <div className="space-y-3">
                {uploadedFiles.map(file => <div key={file.id} className="border border-gray-200 dark:border-gray-700 rounded-lg p-3">
                    ...
                    <div>
                      ...
                      {file.status !== 'uploading' &&
                        <select
                          ...
                          className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(file.status)} border-none focus:ring-1 focus:ring-offset-1`}
                        >
                  ...
                  {file.status === 'processed' && <div className="mt-3 flex justify-end space-x-2">
                    <button onClick={() => handleViewFile(file.id)} className="px-2 py-1 text-xs text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-white">
                      <EyeIcon size={14} className="inline mr-1" />
                      {isRTL ? 'عرض' : 'View'}
                    </button>
                    <button onClick={() => handleCompareFile(file.id)} className="px-2 py-1 text-xs text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-200">
                      <GitCommitIcon size={14} className="inline mr-1" />
                      {isRTL ? 'مقارنة' : 'Compare'}
                    </button>
                    <button onClick={() => handleRemoveFile(file.id)} className="px-2 py-1 text-xs text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-200">
              ...
              {uploadedFiles.length > 0 && <div className="mt-4 pt-3 border-t border-gray-200 dark:border-gray-700">
                <div className="flex justify-between items-center">
                  <div>
                  ...
                  <button className="px-3 py-1.5 text-sm font-medium text-blue-700 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/40 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/60 flex items-center" onClick={onContinue}>
